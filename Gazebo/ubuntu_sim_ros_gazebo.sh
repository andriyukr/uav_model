...
sudo apt-get update
sudo apt-get install git git-core wget zip qtcreator cmake build-essential genromfs ninja-build -y
# Required python packages
sudo apt-get install python-argparse python-dev python-empy python-pip python-toml python-numpy -y
...
then
    ...
    mkdir -p $clone_dir
    cd $clone_dir
    git clone -b v1.6.5 https://github.com/PX4/Firmware.git
    cd Firmware	
    git submodule update --init --recursive
fi
cd $clone_dir/Firmware

echo "export CURRENT_CMAKE_BUILD_DIR="$(catkin locate --workspace ~/catkin_ws/src --build)"" >> ~/.bashrc
echo "source ~/src/Firmware/Tools/setup_gazebo.bash ~/src/Firmware ~/src/Firmware/build/posix_sitl_lpe" >> ~/.bashrc
echo "export GAZEBO_PLUGIN_PATH=${GAZEBO_PLUGIN_PATH}:~/src/Firmware/Tools/sitl_gazebo/Build" >> ~/.bashrc
echo "export GAZEBO_MODEL_PATH=~/src/Firmware/Tools/sitl_gazebo/models" >> ~/.bashrc
echo "export GAZEBO_MODEL_DATABASE_URI=""" >> ~/.bashrc
echo "export SITL_GAZEBO_PATH=~/src/Firmware/Tools/sitl_gazebo" >> ~/.bashrc
echo "export ROS_PACKAGE_PATH=$ROS_PACKAGE_PATH:$HOME/src/Firmware" >> ~/.bashrc
echo "export ROS_PACKAGE_PATH=$ROS_PACKAGE_PATH:$HOME/src/Firmware/Tools/sitl_gazebo" >> ~/.bashrc
